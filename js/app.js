        ...
        function showMore() {
            ...
            window.addEventListener("load", (function(e) {
                ...
                function initItem(showMoreBlock, matchMedia = false) {
                    ...
                    //!Добавление нового функционала/корректировка
 ...
 //!Учет четкой высоты
                ...
                function showMoreActions(e) {
                    ...
                    if (targetType === "click") {
                        if (targetEvent.closest("[data-showmore-button]")) {
                            ...
                            const handleScroll = isCollapse => {
                                if (isCollapse) setTimeout((() => {
                                    showMoreButton.scrollIntoView({
                                        behavior: "smooth",
                                        block: "end"
                                    });
                                }), 600); else setTimeout((() => {
                                    showMoreBlock.style.scrollMarginTop = "150px";
                                    showMoreBlock.scrollIntoView({
                                        behavior: "smooth",
                                        block: "start"
                                    });
                                }), 600);
                            ...
                            if (!showMoreContent.classList.contains("_slide")) if (showLine) {
                                ...
                                const isCollapse = !showMoreBlock.classList.contains("_showmore-active");
        ...
        if (document.querySelector("[data-datepicker]")) {
            const picker = datepicker_min("[data-datepicker]", {
                ...
                customMonths: [ "Jan", "Feb", "Mar", "Apr", "May", "Jum", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" ],
                overlayButton: "Apply",
        ...
        function initSliders() {
            if (document.querySelector(".all-category")) {
                new swiper_core_Swiper(".swiper", {
                    ...
                    pagination: {
                        ...
                        renderBullet: function(index, className) {
                            ...
                            const step = 3;
                            let tpl = "";
                            if (index === 0 || index === total - 1) tpl = "first-last"; else if (index === activeIndex + step && activeIndex + step < total || index === activeIndex - step && activeIndex - step >= 0) tpl = "dots"; else if (index >= activeIndex - 1 && index <= activeIndex + 1) tpl = "visible";
                            switch (tpl) {
                              case "first-last":
                                return `<span class="${className}">${index + 1}</span>`;

                              case "dots":
                                return `<span class="${className} dots" data-index="${index}">...</span>`;
                ...
                document.addEventListener("click", (function(event) {
                    if (event.target.matches(".dots")) {
                        ...
                        const direction = parseInt(event.target.dataset.index) > swiper.activeIndex ? 1 : -1;
                        swiper.slideTo(swiper.activeIndex + step * direction);
                    }
                }));
            }
            ...
            if (document.querySelector(".dessert-category")) {
                new swiper_core_Swiper(".swiper", {
                    modules: [ Navigation, Pagination, Grid ],
                    observer: true,
                    observeParents: true,
        ...
        async function initMap() {
            ...
            const mapElement = document.getElementById("map-delizioso");
            if (!mapElement) return;
            const map = new Map(document.getElementById("map-delizioso"), {
                zoom: 14,
                center,
                mapId: "AIzaSyCiZVu7tK48b6fDBrz2Q_JDNaK3Xq76GPI"
            });
        ...
        function toggleHighlight(markerView, property) {
            if (markerView.classList.contains("highlight")) {
                ...
                markerView.classList.remove("close");
                markerView.classList.add("highlight");
                markerView.zIndex = 1;
            }
        }
        ...
        const properties = [ {
            ...
            address: "1131 Ogden Ave, Bronx, NY 10452, Amerika Serikat",
            description: "Restaurant for the whole family",
            coordinates: "40.885147,-73.9220459",
            social: "s",
            position: {
                lat: 40.88082,
                lng: -73.90863
        ...
        initMap();
        //!Выводит ошибку Google Maps API Not loaded.
                async function initMapCheckout() {
            const {Map} = await google.maps.importLibrary("maps");
            const {AdvancedMarkerElement} = await google.maps.importLibrary("marker");
            const center = {
                lat: 40.88082,
            ...
            const mapElement = document.getElementById("map-checkout");
            if (!mapElement) return;
            const map = new Map(document.getElementById("map-checkout"), {
                zoom: 14,
                center,
                mapId: "AIzaSyCiZVu7tK48b6fDBrz2Q_JDNaK3Xq76GPI"
            });
            for (const property of propertiesCheckout) {
                new google.maps.marker.AdvancedMarkerElement({
                    map,
                    content: buildContentCheckout(property),
                    position: property.position,
                    title: property.description
                });
            }
        }
        function toggleHighlightCheckout(markerView, property) {
            if (markerView.classList.contains("highlight")) {
                markerView.classList.remove("highlight");
                markerView.zIndex = null;
            } else {
        ...
        function buildContentCheckout(property) {
            const content = document.createElement("div");
            content.classList.add("property");
            content.classList.add("close");
            content.innerHTML = `\n\t\t\t\t<img class="user-map-marker" src="img/items_svg/tag-location.svg">\n\t\t\t\t<div class="map-checkout__address-user-map user-map">\n\n        <button type="button" class="address__close-item">l</button>\n\n          <div class="user-map__box-wrap">\n\n            <div class="user-map__point-grid point-grid">\n              <div class="point-grid__col-item col-item">\n\n                <div class="col-item__pic-small">${property.image}</div>\n\n                <div class="col-item__desk-place desk-place">\n                  <div class="desk-place__form-address">\n                    <h2 class="desk-place__title">${property.title}</h2>\n                    <div class="desk-place__position">\n                      <div class="desk-place__bronx">${property.address}</div>\n                      <div class="desk-place__coordinates">${property.coordinates}</div>\n                    </div>\n                  </div>\n                  <div class="desk-place__wrap-confirm">\n                    <button type="button" class="desk-place__confirmation">Confirmation</button>\n                  </div>\n                </div>\n              </div>\n            </div>\n\n          </div>\n \t\t\t</div>`;
            const userMapMarker = content.querySelector(".user-map-marker");
            const userMap = content.querySelector(".user-map");
            userMap.ontransitionend = e => {
                if (e.target != userMap) return;
                if (!content.classList.contains("highlight")) content.classList.add("close");
            };
            userMapMarker.addEventListener("click", (() => {
                toggleHighlightCheckout(content, property);
            }));
            userMap.addEventListener("click", (() => {
                toggleHighlightCheckout(content, property);
            }));
            return content;
        }
        const propertiesCheckout = [ {
            image: '<img src="img/shipping_address_page/highbridge_house.jpg" alt="House" class="col-item__small-icon" width="144" height="205">',
            title: "Highbridge House",
            address: "1131 Ogden Ave, Bronx, NY 10452, Amerika Serikat",
            description: "Restaurant for the whole family",
            coordinates: "40.88232,-73.90458",
            social: "s",
            position: {
                lat: 40.88082,
                lng: -73.90863
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            let imgBx = document.querySelectorAll(".pagination__wrapper-photo");
            let contentBx = document.querySelectorAll(".cell__element");
            for (let i = 0; i < imgBx.length; i++) imgBx[i].addEventListener("mouseover", (function() {
                for (let j = 0; j < contentBx.length; j++) contentBx[j].classList.remove("active");
                document.getElementById(this.dataset.id).classList.add("active");
                for (let j = 0; j < imgBx.length; j++) imgBx[j].classList.remove("active");
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            if (inputField && timePicker && timeWrapper) {
            ...
            function isValidTime(time) {
                ...
                const [minutes, ampm] = minutesAndAmpm.trim().split(" ");
                const hoursNum = parseInt(hours, 10);
                const minutesNum = parseInt(minutes, 10);
                if (isNaN(hoursNum) || isNaN(minutesNum) || ampm !== "AM" && ampm !== "PM") return false;
                if (ampm === "AM") return hoursNum >= 9 && hoursNum <= 11 && minutesNum >= 0 && minutesNum < 60; else if (ampm === "PM") return (hoursNum === 12 || hoursNum >= 1 && hoursNum <= 6) && minutesNum >= 0 && minutesNum < 60;
                return false;
            }
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            function initializePopup() {
                ...
                } else console.warn("Элемент dataOrderAccasion не найден в popup.");
            }
            if (btnBookATable) btnBookATable.addEventListener("click", (function() {
        ...
        //!--------------------------------------------------------------
            ...
            if (window.location.pathname.includes("reservation.html")) {
                const btnBookATable = document.getElementById("btnBookATable");
                const buttonConfirmReservationInForm = document.getElementById("buttonConfirmReservationInForm");
                if (!btnBookATable) {
                    console.error("Элемент с id 'btnBookATable' не найден!");
                    return;
                }
                function initializeCustomSelect() {
                    ...
                    if (!accasionSelectConfirmation) {
                        console.error("Элемент select с id 'requestFrConfirmatiom' не найден!");
                        return;
                    }
                ...
                btnBookATable.addEventListener("click", (function() {
                    buttonConfirmReservationInForm.addEventListener("click", (function() {
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            const visibleInput = document.getElementById("password-visible");
            visibleInput.addEventListener("input", (function(e) {
                const stars = "*".repeat(visibleInput.value.length);
                visibleInput.value = stars;
                passwordInput.value = e.target.value;
            }));
            visibleInput.addEventListener("keydown", (function(e) {
                if (e.key === "Backspace" || e.key === "Delete") {
                    const originalLength = passwordInput.value.length;
                    if (originalLength > 0) passwordInput.value = passwordInput.value.slice(0, -1);
                }
            }));
        }));
        document.addEventListener("DOMContentLoaded", (function() {
        ...
        const menu = {
            initMenuPopreserv: function() {
                const reservationButton = document.querySelector(".form-page__btn-reservation");
                if (reservationButton) reservationButton.addEventListener("click", this.handleReservationButtonClick);
                const menuIcon = document.querySelector(".menu-popreserv__icon");
                if (menuIcon) menuIcon.addEventListener("click", (() => this.toggleMenu()));
            },
            toggleMenu: function() {
        ...
        function initPopupForm() {
            ...
            if (accasionFantomList) accasionFantomList.addEventListener("click", (function(event) {
                ...
                validateForm();
            })); else console.warn("Element accasionFantomList не найден.");
